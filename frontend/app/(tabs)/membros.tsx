...
function confirmAction(message: string, onConfirm: () => void, destructive = false) {
  if (Platform.OS === 'web') {
    ...
    Alert.alert(
      destructive ? 'Confirmar' : 'Confirmar',
      message,
      [
...
export default function MembrosScreen() {
  ...
  const toggleRole = (m: Member) => {
    ...
    const msg = newRole === 'leader'
      ? `Promover ${m.name} a líder? Terá acesso total ao ministério.`
      : `Rebaixar ${m.name} a membro? Perderá os privilégios de líder.`;

    confirmAction(msg, async () => {
      setActing(m.id);
      try {
        const updated = await api<Member>(`/ministry/members/${m.id}`, {
          method: 'PUT',
          body: { role: newRole },
        });
        ...
        showError(e.message || 'Falha ao atualizar membro');
      } finally {
        setActing(null);
      }
    });
  ...
  const removeMember = (m: Member) => {
    confirmAction(
      `Remover ${m.name} permanentemente do ministério?`,
      async () => {
        setActing(m.id);
        try {
          ...
          showError(e.message || 'Falha ao remover membro');
        } finally {
          setActing(null);
        }
      },
